...
void Fence::drawLine(int o, int p1, int p2, int c) { // o = 0, horizontal, else vertical // from p1 to p2, c is center point, on axis perpindicular to the fence
	...
	if (!o) { // horizontal	
		...
		int needed_links = ceil((double)(p2 - p1)/(double)(links.front()->getLengthX() - 30)); // find the needed links for the this gap
			//should overlap somewhat, so I subtract 30
		...
		int needed_links = ceil((double)(p2 - p1)/(double)(links.front()->getLengthY() - 30)); // find the needed links for the this gap
			// should overlap
...
void Fence::drawBox(int x, int y, int l, int w) {
	...
	drawLine(1, y + links.front()->getBuffer(), y + l - links.front()->getBuffer(), x - links.front()->getBuffer()/3); // left line
	...
	drawLine(1, y + links.front()->getBuffer(), y + l - links.front()->getBuffer(), x + w + links.front()->getBuffer()/3); // right line
	links.push_front(new FenceLink(2, x + w, y, screenWidth, screenHeight)); // top right corner
	drawLine(0, x + links.front()->getBuffer(), x + w - links.front()->getBuffer(), y - links.front()->getBuffer()/3); // top line
}
